...
export const registerService = ({ phone, name, password }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Đăng ký tài khoản thành công!!' : 'Tài khoản đã tồn tại! Vui lòng nhập số điện thoại khác.',
            token: token || null
        })
    } catch (error) {
        reject(error)
...
export const loginService = ({ phone, password }) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.User.findOne({
            where: { phone },
            raw: true // để raw là true để respone trả về là object để lấy dữ liệu
        })

        //hàm trên sẽ trả về 1 object 
        ...
        resolve({
            err: token ? 0 : 2,
            msg: token ? 'Đăng nhập thành công!!' : response ? 'Mật khẩu không đúng!' : 'Số điện thoại không đúng!',
            token: token || null
        })
    } catch (error) {
        reject(error)
